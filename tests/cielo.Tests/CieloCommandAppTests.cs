...
public sealed class CieloCommandAppTests
{
    ...
    {
        var message = new PendingMessage(new Dictionary<string, object?>
        {
            ["actionType"] = "mode",
            ["actionValue"] = "heat"
        }, "mode heat");
    ...
    {
        var parseResult = CieloCommandApp.BuildRootCommand().Parse(["monitor", "--daemon", "--interval", "60", "--samples", "1"]);

        Assert.Contains(parseResult.Errors, error => error.Message == "--daemon cannot be combined with --samples.");
    }

    [Fact]
    public void BuildRootCommand_AllowsMonitorDaemonWithoutSamples()
    {
        var parseResult = CieloCommandApp.BuildRootCommand().Parse(["monitor", "--daemon", "--interval", "60"]);
