...
internal sealed class MonitorRulesConfig
{
    ...
    public void Validate()
    {
        if (Version != 1)
        {
            throw new InvalidOperationException($"Unsupported rules version '{Version}'. Expected version 1.");
        }
        ...
        for (var index = 0; index < Rules.Count; index++)
        {
            ...
            {
                throw new InvalidOperationException($"Duplicate rule name '{rule.Name}'. Rule names must be unique.");
            }
        }
    }
}
...
internal sealed class PlanDefaults
{
    ...
    public void Validate()
    {
        ...
        {
            throw new InvalidOperationException($"planDefaults.minSetpoint ({MinSetpoint}) must be less than maxSetpoint ({MaxSetpoint}).");
        }
    }
}
...
internal sealed class MonitorRule
{
    ...
    public void Validate(int index)
    {
        ...
        {
            throw new InvalidOperationException($"Rule '{Name}' has a negative cooldownSeconds value.");
        }
    }
}
...
internal sealed class MonitorRuleActiveWindow
{
    ...
    {
        if (string.IsNullOrWhiteSpace(Timezone) || string.IsNullOrWhiteSpace(Start) || string.IsNullOrWhiteSpace(End))
        {
            throw new InvalidOperationException($"Rule '{ruleName}' active window requires timezone, start, and end.");
        }
        ...
        {
            throw new InvalidOperationException($"Rule '{ruleName}' uses unknown timezone '{Timezone}'.");
        }
        catch (InvalidTimeZoneException)
        {
            throw new InvalidOperationException($"Rule '{ruleName}' uses invalid timezone '{Timezone}'.");
        }
        ...
        if (start == end)
        {
            throw new InvalidOperationException($"Rule '{ruleName}' active window start and end must differ.");
        }
    }
    ...
    private static TimeOnly ParseTime(string ruleName, string value, string fieldName)
    {
        ...
        {
            throw new InvalidOperationException($"Rule '{ruleName}' uses invalid active.{fieldName} value '{value}'. Expected HH:mm.");
        }
...
internal sealed class MonitorRuleCondition
{
    ...
    public void Validate(string ruleName)
    {
        if (!string.Equals(Metric, "roomTemp", StringComparison.OrdinalIgnoreCase) &&
        ...
        {
            throw new InvalidOperationException($"Rule '{ruleName}' uses unsupported metric '{Metric}'. Supported metrics: roomTemp, targetTemp, humidity.");
        }

        if ((Below is null && Above is null) || (Below is not null && Above is not null))
        {
            throw new InvalidOperationException($"Rule '{ruleName}' must define exactly one of 'below' or 'above'.");
        }
        ...
        if (string.Equals(Metric, "humidity", StringComparison.OrdinalIgnoreCase) && !string.IsNullOrWhiteSpace(Unit))
        {
            throw new InvalidOperationException($"Rule '{ruleName}' should not specify a unit for humidity.");
        }
    }
}
