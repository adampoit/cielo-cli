...
internal sealed class CieloWebSocketSession : IAsyncDisposable
{
	...
	public static async Task<CieloWebSocketSession> ConnectAsync(
		string sessionId,
		string accessToken,
	...
	)
	{
		...
		session._socket.Options.SetRequestHeader(
			"User-agent",
		...
		var uri = new Uri(
			$"wss://apiwss.smartcielo.com/websocket/?sessionId={Uri.EscapeDataString(sessionId)}&token={Uri.EscapeDataString(accessToken)}"
		);
	...
	)
	{
		...
		try
		{
			...
			{
				var message =
					await ReceiveJsonAsync(timeoutSource.Token)
					?? throw new InvalidOperationException(
						"Websocket closed before Cielo acknowledged the command."
					);

				var root = message.RootElement;
				trace?.Invoke($"<<< {root.GetRawText()}");
				var currentMac =
					TryGetString(root, "mac_address") ?? TryGetString(root, "macAddress");
		...
		catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
		{
			throw new TimeoutException(
				$"Timed out waiting for Cielo to acknowledge the update for {macAddress}."
			);
		}
	}
