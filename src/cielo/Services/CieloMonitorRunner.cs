...
internal sealed class CieloMonitorRunner
{
	...
	)
	{
		...
		var (rulesConfig, resolvedRulesPath) = await MonitorRulesStore.LoadOptionalAsync(
			options.RulesPath,
			cancellationToken
		);
		...
		{
			targetMacAddress = CieloDeviceResolver
				.Resolve(initialDevices, options.Device!)
				.MacAddress;
		}
		...
		var weatherClient =
			options.WeatherLatitude.HasValue
			&& options.WeatherLongitude.HasValue
			&& initialSelectedDevices.Count > 0
		...
		var rulesEngine =
			rulesConfig is null || resolvedRulesPath is null
				? null
				: new CieloMonitorRulesEngine(
					rulesConfig,
					resolvedRulesPath,
					options.DryRun,
					rulesEnvironment
			...
			while (!cancellationToken.IsCancellationRequested)
			{
				...
				if (selectedDevices.Count == 0)
				{
					throw new InvalidOperationException(
						"The selected device was not returned by Cielo during monitoring."
					);
				}
				...
				{
					var sample = MonitorSample.FromDevice(
						device,
						timestamp,
						"poll",
						outdoorWeather
					...
					if (!string.IsNullOrWhiteSpace(historyFilePath))
					{
						await File.AppendAllTextAsync(
							historyFilePath,
							CieloOutput.ToMonitorSampleJson(sample) + Environment.NewLine,
							cancellationToken
						);
					}
				...
				completedSamples++;
				if (
					options.SampleLimit is not null
					&& completedSamples >= options.SampleLimit.Value
				)
				{
					break;
				}
