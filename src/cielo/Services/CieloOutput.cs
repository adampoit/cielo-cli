...
internal static class CieloOutput
{
    ...
    public static void WriteDevices(IReadOnlyList<CieloDevice> devices, bool json)
    {
        ...
        foreach (var device in devices.OrderBy(device => device.DeviceName, StringComparer.OrdinalIgnoreCase))
        {
            ...
            Console.WriteLine($"{device.DeviceName} | {device.MacAddress} | {device.LatestAction.Power} {device.NormalizedMode} {device.LatestAction.Temperature}{device.TemperatureUnit} | room {room} | {(device.IsOnline ? "online" : "offline")}");
        }
    }
    ...
    public static void WriteDeviceStatus(CieloDevice device, bool json)
    {
        ...
        Console.WriteLine($"Target Temp: {device.LatestAction.Temperature}{device.TemperatureUnit}");
        ...
        {
            Console.WriteLine($"Room Temp: {device.Environment.Temperature}{device.TemperatureUnit}");
        }
    ...
    public static void WriteCommandResult(string summary, JsonDocument response, bool json)
    {
        ...
        var macAddress = TryGetString(response.RootElement, "mac_address") ?? TryGetString(response.RootElement, "macAddress") ?? "unknown-device";
    ...
    public static void WriteMonitorSample(MonitorSample sample, bool json)
    {
        ...
        var humidity = sample.Humidity is null
            ? "n/a"
            : $"{sample.Humidity}%";
        var outdoor = sample.OutdoorTemp is null
            ? string.Empty
            : $", outdoor {sample.OutdoorTemp}{sample.OutdoorTempUnit}" +
              (sample.OutdoorApparentTemp is null ? string.Empty : $" feels {sample.OutdoorApparentTemp}{sample.OutdoorTempUnit}") +
              (sample.OutdoorWindSpeed is null ? string.Empty : $", wind {sample.OutdoorWindSpeed}{sample.OutdoorWindSpeedUnit}");
        Console.WriteLine($"[{sample.Timestamp:u}] {sample.DeviceName}: room {roomTemp}, humidity {humidity}, target {sample.TargetTemp}{sample.TargetTempUnit}, {sample.Power} {sample.Mode}, fan {sample.Fan}{outdoor}");
    }
    ...
    public static bool ShouldWriteMessage(JsonElement message, string? targetMacAddress)
    {
        ...
        var macAddress = TryGetString(message, "mac_address") ?? TryGetString(message, "macAddress");
        return string.Equals(macAddress, targetMacAddress, StringComparison.OrdinalIgnoreCase);
    }

    public static void WriteWatchMessage(JsonElement message, IReadOnlyDictionary<string, CieloDevice> devices, bool json)
    {
        if (json)
        {
        ...
        var macAddress = TryGetString(message, "mac_address") ?? TryGetString(message, "macAddress") ?? string.Empty;
        var deviceName = devices.TryGetValue(macAddress, out var device) ? device.DeviceName : macAddress;
        var timestamp = DateTimeOffset.Now.ToString("u");

        if (string.Equals(messageType, "StateUpdate", StringComparison.OrdinalIgnoreCase))
        {
            var action = message.TryGetProperty("action", out var actionElement) ? actionElement : default;
            var latEnv = message.TryGetProperty("lat_env_var", out var latEnvElement) ? latEnvElement : default;
            ...
            Console.WriteLine($"[{timestamp}] {deviceName}: {power} {mode}, target {temp}, room {roomTemp}{humidityPart}");
            return;
        }
