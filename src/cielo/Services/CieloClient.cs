...
internal sealed class CieloClient : IDisposable
{
	...
	public static readonly string[] ValidSwingModes =
	[
		"auto",
		"auto/stop",
		"adjust",
	...
	public async Task RefreshTokenAsync(CancellationToken cancellationToken)
	{
		...
		{
			Content = JsonContent.Create(new { local = "en", refreshToken = _config.RefreshToken }),
		};
		...
		var payload =
			await response.Content.ReadFromJsonAsync<ApiEnvelope<RefreshTokenResponse>>(
				JsonOptions,
				cancellationToken
			)
			?? throw new InvalidOperationException(
				"Cielo token refresh returned an empty response."
			);

		if (
			payload.Status != 200
			|| !string.Equals(payload.Message, "SUCCESS", StringComparison.OrdinalIgnoreCase)
		...
		var expiresAt = DateTimeOffset.FromUnixTimeSeconds(
			Math.Max(payload.Data.ExpiresIn - 300, 0)
		);
	...
	public async Task<IReadOnlyList<CieloDevice>> GetDevicesAsync(
	...
	)
	{
		...
		var payload =
			await response.Content.ReadFromJsonAsync<ApiEnvelope<DeviceListResponse>>(
				JsonOptions,
				cancellationToken
			)
			?? throw new InvalidOperationException(
				"Cielo device discovery returned an empty response."
			);
		...
		{
			throw new InvalidOperationException(
				$"Cielo device discovery failed: {payload.Message}"
			);
		}

		var devices = payload.Data.ListDevices.Where(device => device.ApplianceId > 0).ToList();

		var applianceMap = await GetAppliancesAsync(
			devices.Select(device => device.ApplianceId).Distinct(),
			cancellationToken
		);
	...
	public async Task<CieloWebSocketSession> OpenWebSocketAsync(CancellationToken cancellationToken)
	{
		...
		return await CieloWebSocketSession.ConnectAsync(
			_config.SessionId,
			_config.AccessToken,
			cancellationToken
		);
	...
	public IReadOnlyList<PendingMessage> BuildActionControlMessages(
		CieloDevice device,
	...
	)
	{
		...
		var needPower =
			power is not null
			&& !string.Equals(power, device.LatestAction.Power, StringComparison.OrdinalIgnoreCase);
		...
		var needTemp =
			changes.Temperature is not null
			&& (
				!int.TryParse(device.LatestAction.Temperature, out var currentTemp)
				|| currentTemp != changes.Temperature.Value
			);
		var needFan =
			fan is not null
			&& !string.Equals(
		...
		var needSwing =
			swing is not null
			&& !string.Equals(swing, device.LatestAction.Swing, StringComparison.OrdinalIgnoreCase);
	...
	private async Task<Dictionary<long, CieloAppliance>> GetAppliancesAsync(
		IEnumerable<long> applianceIds,
		CancellationToken cancellationToken
	)
	{
		...
		using var request = new HttpRequestMessage(
			HttpMethod.Get,
			$"{ApiHost}/web/sync/db/6?applianceIdList={encodedList}"
		);
		AddAuthHeaders(request);
		...
		var payload =
			await response.Content.ReadFromJsonAsync<ApiEnvelope<ApplianceListResponse>>(
				JsonOptions,
				cancellationToken
			)
			?? throw new InvalidOperationException(
				"Cielo appliance metadata returned an empty response."
			);
		...
		{
			throw new InvalidOperationException(
				$"Cielo appliance metadata lookup failed: {payload.Message}"
			);
		}
	...
	private PendingMessage BuildTemperatureMessage(CieloDevice device, int temperature)
	{
		...
		var currentValue = int.TryParse(device.LatestAction.Temperature, out var parsedCurrent)
			? parsedCurrent
			: temperature;
	...
	private PendingMessage BuildPresetMessage(CieloDevice device, string preset)
	{
		...
		throw new InvalidOperationException(
			$"Preset '{preset}' was not found on {device.DeviceName}."
		);
	}
	...
	)
	{
		var message = new Dictionary<string, object?>(StringComparer.Ordinal)
		{
			...
			["myRuleConfiguration"] =
				device.MyRuleConfiguration ?? JsonDocument.Parse("{}").RootElement,
			["preset"] = preset,
			["actions"] = action,
			["oldPower"] = oldPower,
		};
	...
	{
		if (
			string.Equals(preset, "turbo", StringComparison.OrdinalIgnoreCase)
