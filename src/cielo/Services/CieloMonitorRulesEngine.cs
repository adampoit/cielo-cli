...
internal sealed class CieloMonitorRulesEngine
{
    ...
    private readonly Dictionary<string, RuleRuntimeState> _state = new(StringComparer.OrdinalIgnoreCase);
        ...
        IReadOnlyDictionary<string, string>? extraEnvironment = null)
    {
        ...
        _dryRun = dryRun;
        _extraEnvironment = extraEnvironment ?? new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
    }

    public void WriteStartupMessage()
    {
        Console.Error.WriteLine($"Loaded {_config.Rules.Count} rule(s) from {_rulesPath}{(_dryRun ? " (dry-run)" : string.Empty)}.");
    }
        ...
        foreach (var rule in _config.Rules)
        {
            ...
            if (state.InAlert)
            {
                if (HasRecovered(rule.When, sample, metric))
                {
                    state.InAlert = false;
                    state.ConsecutiveMatches = 0;
                    Console.Error.WriteLine($"Rule '{rule.Name}' re-armed for {sample.DeviceName}.");
                    continue;
                }

                if (IsTriggered(rule.When, metric) &&
                    ShouldFireInAlert(rule, sample, state))
                {
            ...
            if (IsTriggered(rule.When, metric))
            {
                ...
                if (state.ConsecutiveMatches >= rule.When.ForSamples)
                {
                    ...
                    if (state.LastTriggeredAt is not null &&
                        sample.Timestamp - state.LastTriggeredAt.Value < TimeSpan.FromSeconds(rule.CooldownSeconds))
                    {
                        Console.Error.WriteLine($"Rule '{rule.Name}' matched for {sample.DeviceName} but is cooling down.");
                        continue;
                    }
    ...
    private async Task FireRuleAsync(MonitorRule rule, MonitorSample sample, RuleRuntimeState state)
    {
        ...
        {
            Console.Error.WriteLine($"Rule '{rule.Name}' skipped for {sample.DeviceName} because the previous action is still running.");
            return;
        }

        state.LastTriggeredAt = sample.Timestamp;
        var renderedArgs = rule.Action.Args.Select(arg => RenderTemplate(arg, rule, sample)).ToArray();
        var summary = $"Rule '{rule.Name}' matched for {sample.DeviceName}: {DescribeCondition(rule.When, sample)}";

        if (_dryRun)
        {
            Console.Error.WriteLine($"{summary} [dry-run] -> {rule.Action.Exec} {string.Join(' ', renderedArgs)}");
            return;
        }

        Console.Error.WriteLine($"{summary} -> {rule.Action.Exec} {string.Join(' ', renderedArgs)}");
        state.RunningTask = RunActionAsync(rule, sample, renderedArgs, _rulesPath, _extraEnvironment);
        await ObserveCompletedExecutionAsync(state);
    }
        ...
        IReadOnlyDictionary<string, string> extraEnvironment)
    {
        try
        {
            ...
            var stdinPayload = JsonSerializer.Serialize(new
            {
                ...
                historyFile = extraEnvironment.TryGetValue("CIELO_HISTORY_FILE", out var historyFile) ? historyFile : null,
            ...
            Console.Error.WriteLine($"Rule '{rule.Name}' action exited with code {process.ExitCode}.");
        }
        catch (Exception ex)
        {
        ...
        IReadOnlyDictionary<string, string> extraEnvironment)
    {
        ...
        startInfo.Environment["CIELO_OUTDOOR_TEMP"] = sample.OutdoorTemp?.ToString() ?? string.Empty;
        startInfo.Environment["CIELO_OUTDOOR_HUMIDITY"] = sample.OutdoorHumidity?.ToString() ?? string.Empty;
        startInfo.Environment["CIELO_OUTDOOR_APPARENT_TEMP"] = sample.OutdoorApparentTemp?.ToString() ?? string.Empty;
        startInfo.Environment["CIELO_OUTDOOR_WIND_SPEED"] = sample.OutdoorWindSpeed?.ToString() ?? string.Empty;
        startInfo.Environment["CIELO_OUTDOOR_WEATHER_CODE"] = sample.OutdoorWeatherCode?.ToString() ?? string.Empty;
        startInfo.Environment["CIELO_OUTDOOR_TEMP_UNIT"] = sample.OutdoorTempUnit ?? string.Empty;
        startInfo.Environment["CIELO_OUTDOOR_WIND_SPEED_UNIT"] = sample.OutdoorWindSpeedUnit ?? string.Empty;
    ...
    private static bool AppliesTo(MonitorRule rule, MonitorSample sample)
    {
        ...
        return string.Equals(rule.Device, sample.DeviceName, StringComparison.OrdinalIgnoreCase) ||
               ...
               string.Equals(rule.Device, sample.ApplianceId.ToString(), StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static bool TryGetMetricValue(MonitorRuleCondition condition, MonitorSample sample, out decimal? value)
    {
    ...
    private static bool HasRecovered(MonitorRuleCondition condition, MonitorSample sample, decimal metricValue)
    {
    ...
    private static bool ShouldFireInAlert(MonitorRule rule, MonitorSample sample, RuleRuntimeState state)
    {
        return state.LastTriggeredAt is null ||
               sample.Timestamp - state.LastTriggeredAt.Value >= TimeSpan.FromSeconds(rule.CooldownSeconds);
    }

    private static decimal GetDefaultHysteresis(MonitorRuleCondition condition, MonitorSample sample)
    {
        ...
        return string.Equals(sample.TargetTempUnit, "C", StringComparison.OrdinalIgnoreCase) ? 0.5m : 1m;
    }
    ...
    private static string DescribeCondition(MonitorRuleCondition condition, MonitorSample sample)
    {
        ...
        var current = string.Equals(condition.Metric, "roomTemp", StringComparison.OrdinalIgnoreCase)
    ...
    private static string RenderTemplate(string template, MonitorRule rule, MonitorSample sample)
    {
        return template
            ...
            .Replace("{{applianceId}}", sample.ApplianceId.ToString(), StringComparison.OrdinalIgnoreCase)
            .Replace("{{roomTemp}}", sample.RoomTemp?.ToString() ?? string.Empty, StringComparison.OrdinalIgnoreCase)
            .Replace("{{humidity}}", sample.Humidity?.ToString() ?? string.Empty, StringComparison.OrdinalIgnoreCase)
            .Replace("{{targetTemp}}", sample.TargetTemp, StringComparison.OrdinalIgnoreCase)
            .Replace("{{targetTempUnit}}", sample.TargetTempUnit, StringComparison.OrdinalIgnoreCase)
