...
internal static class CieloDeviceResolver
{
    public static CieloDevice Resolve(IReadOnlyList<CieloDevice> devices, string identifier)
    {
        ...
        if (long.TryParse(normalized, out var applianceId))
        {
            ...
            if (byApplianceId.Count > 1)
            {
                throw new InvalidOperationException($"Appliance id '{identifier}' matches multiple devices. Use the MAC address instead.");
            }
        }

        var byName = devices.Where(device =>
            string.Equals(device.DeviceName, normalized, StringComparison.OrdinalIgnoreCase)).ToList();
        ...
        if (byName.Count > 1)
        {
            throw new InvalidOperationException($"Device name '{identifier}' matches multiple devices. Use the MAC address instead.");
        }
