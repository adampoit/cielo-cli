...
internal sealed class OpenMeteoWeatherClient
{
    ...
    public OpenMeteoWeatherClient(double latitude, double longitude, string temperatureUnit, TimeSpan refreshInterval)
    {
        _latitude = latitude;
    ...
    public async Task<OutdoorWeatherSnapshot?> GetCurrentAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException("Open-Meteo response did not contain current weather data.");
            }

            _cachedSnapshot = new OutdoorWeatherSnapshot(
                DateTimeOffset.TryParse(response.Current.Time, out var timestamp) ? timestamp : DateTimeOffset.UtcNow,
