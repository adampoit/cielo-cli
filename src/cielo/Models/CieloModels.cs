...
internal sealed class CieloDevice
{
    ...
    public bool IsOnline =>
        ...
        DeviceStatusRaw.ValueKind == JsonValueKind.String && string.Equals(DeviceStatusRaw.GetString(), "on", StringComparison.OrdinalIgnoreCase);

    [JsonIgnore]
    public bool IsBreezMax => string.Equals(DeviceType, "BREEZ-MAX", StringComparison.OrdinalIgnoreCase);

    [JsonIgnore]
    public string NormalizedMode => string.Equals(LatestAction.Mode, "mode", StringComparison.OrdinalIgnoreCase)
        ? "cool"
        : LatestAction.Mode;
    ...
    [JsonIgnore]
    public bool SupportsTargetTemperature => !string.Equals(Appliance?.TemperatureRange, "inc:dec", StringComparison.OrdinalIgnoreCase);
